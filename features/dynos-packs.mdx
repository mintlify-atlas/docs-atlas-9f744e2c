---
title: "Toggle DynOS resource packs in-launcher"
sidebarTitle: "DynOS Packs"
description: "DynOS packs are subdirectories under ~/.local/share/sm64coopdx/dynos/packs/. Enable or disable them from the launcher's DynOS Packs panel without touching config files."
---
...
Each **immediate subdirectory** inside `packs/` is treated as one pack. Plain files and nested directories without a valid pack structure are silently ignored. The launcher creates the `dynos/packs/` directory automatically on first run if it does not already exist.
...
Enabled packs are recorded as `dynos-pack: <name>` lines in `sm64config.txt`, where `<name>` is the subdirectory name inside `dynos/packs/`:
